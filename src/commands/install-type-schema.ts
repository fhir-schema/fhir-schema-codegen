...
export class InstallTypeSchemaCommand extends BaseCommand {
  register(program: Commander): void {
    program
      ...
      .addArgument(
        new Argument('[version]', 'Specific version to install').default(TYPE_SCHEMA_VERSION, 'default version')
      )
      .action(async (version: string) => {
        try {
          ...
          logger.error(`Failed to install type-schema binary: ${error instanceof Error ? error.message : String(error)}`);
          process.exit(1);
        }
      });
  }
