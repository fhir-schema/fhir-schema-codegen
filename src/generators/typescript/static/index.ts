...
export class Client<T extends BasicAuthorization | ResourceOwnerAuthorization> {
    ...
    resource = {
        processBundle: (data: ResourceTypeMap['Bundle']): Promise<ResourceTypeMap['Bundle']> => {
            return this.client.post(buildResourceUrl(''), { json: data }).json<ResourceTypeMap['Bundle']>();
        },
        conditionalUpdate: <T extends keyof ResourceTypeMap>(resourceName: T, input: PartialResourceBody<T>) => {
        ...
        conditionalCreate: <T extends keyof ResourceTypeMap>(resourceName: T, input: PartialResourceBody<T>) => {
            return new FindBeforeAction((searchParams: URLSearchParams) => {
                return this.client.post(buildResourceUrl(resourceName), { json: input, searchParams });
            }, resourceName);
        },
        conditionalOverride: <T extends keyof ResourceTypeMap>(resourceName: T, input: PartialResourceBody<T>) => {
            return new FindBeforeAction((searchParams: URLSearchParams) => {
        ...
        update: <T extends keyof ResourceTypeMap>(
            ...
            return this.client.patch(buildResourceUrl(resourceName, id), { json: input }).json<ResourceTypeMap[T]>();
        },
        create: <T extends keyof ResourceTypeMap>(
            resourceName: T,
            input: SetOptional<ResourceTypeMap[T] & { resourceType: string }, 'resourceType'>
        ) => {
            return this.client.post(buildResourceUrl(resourceName), { json: input }).json<ResourceTypeMap[T]>();
        },
        override: <T extends keyof ResourceTypeMap>(
            ...
            return this.client.put(buildResourceUrl(resourceName, id), { json: input }).json<ResourceTypeMap[T]>();
        },
    };
