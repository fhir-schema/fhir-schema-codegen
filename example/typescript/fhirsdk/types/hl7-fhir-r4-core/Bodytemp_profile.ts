...
export const attach_Bodytemp = (resource: Observation, profile: Bodytemp): Observation => {
    return {
        ...resource,
        meta: {
...
export const extract_Observation = (resource: Observation): Bodytemp => {
    if (resource.category === undefined) {
        throw new Error("'category' is required for http://hl7.org/fhir/StructureDefinition/bodytemp");
    }
    
    if (resource.subject === undefined) {
        throw new Error("'subject' is required for http://hl7.org/fhir/StructureDefinition/bodytemp");
    }
    ...
    return {
        ...
        subject: resource.subject as Bodytemp['subject'],
